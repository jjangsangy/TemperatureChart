...
- **Dynamic Y-Axis Scaling**: The Y-axis scaling in `src/components/temperature-chart.tsx` now dynamically adjusts based on the selected hourly variable, calculating padded minimum and maximum values to provide a user-friendly scale.
- **Smoother Loading Transitions**: The loading experience has been enhanced with a more structured loading card and a `MetadataSkeleton` component, providing smoother transitions.
